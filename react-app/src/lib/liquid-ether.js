// Vanilla-JS port of the LiquidEther WebGL fluid background (React Bits).
// Framework-agnostic module wrapped by src/components/LiquidEtherBackground.jsx
// for use inside the React app (bundled via the npm `three` package).
import * as THREE from 'three';

const defaultColors = ['#4ea5ff', '#7cbcff', '#1d2f47'];

/**
 * Mounts the LiquidEther fluid simulation into `container` and starts it.
 * Returns a controller object with a `destroy()` method for cleanup.
 */
export function createLiquidEther(container, options = {}) {
  ...
  // Transparent background vector (alpha 0) so it blends over the page background.
